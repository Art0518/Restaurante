...
namespace FacturacionService.Controllers
{
    ...
    public class FacturacionController : ControllerBase
    {
        ...
        // NUEVO: POST /api/facturas/proxy-banco
        [HttpPost("proxy-banco")]
        public async Task<IActionResult> ProxyBanco([FromBody] ProxyBancoRequest request)
        {
            try
            {
                using var httpClient = new HttpClient();
                var bancoUrl = "http://mibanca.runasp.net/api/Transacciones";
                var content = new StringContent(
                    JsonSerializer.Serialize(request),
                    Encoding.UTF8,
                    "application/json"
                );
                var response = await httpClient.PostAsync(bancoUrl, content);
            ...
            {
                return StatusCode(500, new { ok = false, mensaje = "Error en el proxy", error = ex.Message });
            }
        }
    }
}
